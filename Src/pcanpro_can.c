...
volatile struct fw_diag_state g_fw_diag =
{
  .magic = 0x46574442u,
};
...
static int _can_send( FDCAN_HandleTypeDef *p_can, struct t_can_msg *p_msg )
{
	...
	g_fw_diag.can_hw_send_attempts++;
	g_fw_diag.last_tx_id = p_msg->id;
	g_fw_diag.last_tx_size_flags = ((uint32_t)p_msg->size & 0xffffu) | ((uint32_t)p_msg->flags << 16);
...
{
	eFeedback eRet = FBK_Success;
	FDCAN_HandleTypeDef *p_can = (bus < CAN_BUS_TOTAL) ? can_dev_array[bus].dev : NULL;

	g_fw_diag.bus_active_calls++;
	PRINT_DEBUG("pcan_can_set_bus_active(bus=%d, mode=%u) dev=%p", bus, mode, p_can);
...
void pcan_can_poll( void )
{
  ...
  if( (ts_ms - err_last_check) > 2000 )
  {
    ...
    for( int i = 0; i < CAN_BUS_TOTAL; i++ )
    {
      ...
      HAL_FDCAN_GetProtocolStatus(pcan, &psr);
      FDCAN_ErrorCountersTypeDef ecr;
      HAL_FDCAN_GetErrorCounters(pcan, &ecr);
      g_fw_diag.last_can_psr = pcan->Instance->PSR;
      g_fw_diag.last_can_ir = pcan->Instance->IR;
      if( psr.BusOff || psr.ErrorPassive || ecr.TxErrorCnt > 0 || ecr.RxErrorCnt > 0 )
      {
        PRINT_DEBUG("CAN%d: BusOff=%d ErrPassive=%d TxErr=%lu RxErr=%lu LastErr=%d",
          i, psr.BusOff, psr.ErrorPassive, ecr.TxErrorCnt, ecr.RxErrorCnt, psr.LastErrorCode);
      }
    }
  }
}
