...
static uint8_t device_data_in( USBD_HandleTypeDef *pdev, uint8_t epnum )
{
  ...
  if( epnum == (PCAN_USB_EP_MSGIN_CH1 & EP_ADDR_MSK) )
  {
    g_fw_diag.usb_data_in_complete_count++;
    g_fw_diag.usb_data_in_last_bytes = pdev->ep_in[epnum].total_length;
  }
...
static uint8_t device_data_out( USBD_HandleTypeDef *pdev, uint8_t epnum )
{
  ...
  g_fw_diag.usb_out_last_ep = epnum;
  g_fw_diag.usb_out_last_size = (uint32_t)size;

  if( epnum == PCAN_USB_EP_CMDOUT )
  {
    g_fw_diag.usb_out_ep1_packets++;
