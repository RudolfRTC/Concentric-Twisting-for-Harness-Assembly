...
{
    PRINT_DEBUG("can_open(mode=%lu) is_open=%d Brp=%u Seg1=%u Seg2=%u Sjw=%u",
        mode, can_is_open, can_bitrate_nominal.Brp,
        can_bitrate_nominal.Seg1, can_bitrate_nominal.Seg2, can_bitrate_nominal.Sjw);
    ...
    {
        PRINT_FAULT("HAL_FDCAN_Init FAILED err=0x%lx", can_handle.ErrorCode);
        return FBK_ErrorFromHAL; // error detail in can_handle.ErrorCode
    }
    PRINT_DEBUG("HAL_FDCAN_Init OK");
    ...
    {
        PRINT_FAULT("HAL_FDCAN_Start FAILED err=0x%lx", can_handle.ErrorCode);
        return FBK_ErrorFromHAL;
    }

    can_is_open = true;
    PRINT_DEBUG("CAN bus OPEN - mode=%lu", mode);
    return FBK_Success;
}
...
void can_process(uint32_t tick_now)
{
    ...
    g_fw_diag.last_can_psr = can_handle.Instance->PSR;
    g_fw_diag.last_can_ir = can_handle.Instance->IR;
