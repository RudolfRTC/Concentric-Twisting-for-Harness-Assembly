...
int pcan_flush_data( struct t_m2h_fsm *pfsm, void *src, int size )
{
  ...
  switch( pfsm->state )
  {
    ...
    case 0:
      ...
      if( pfsm->ep_addr == PCAN_USB_EP_MSGIN_CH1 )
      {
        g_fw_diag.usb_data_in_submit_count++;
        g_fw_diag.usb_data_in_submit_bytes += (uint32_t)size;
        g_fw_diag.usb_data_in_last_bytes = (uint32_t)size;
      }
